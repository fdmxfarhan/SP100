/* Includes *******************************************************************/
#include "SP100_Intake.h"
#include "SP100_TLE94108ES.h"

/* Private typedef ************************************************************/
/* Private define *************************************************************/
/* Private macro **************************************************************/
/* Private variables **********************************************************/

/* Functions body *************************************************************/
/* SP100_Intake_STP ---------------------------------------------------------*/
void SP100_Intake_STP(void)
{
	uint8_t temp;
	temp = SP100_TLE_Ctrl_Register_Read(HB_ACT_2_CTRL);
	if(temp != 0x00)
	{
		SP100_TLE_Ctrl_Register_Write(HB_ACT_2_CTRL, 0x00);		//--> OUTL5:HZ & OUTL6:HZ
	}
}

/* SP100_Intake_Fresh -------------------------------------------------------*/
void SP100_Intake_Fresh(void)
{
	uint8_t temp;
	temp = SP100_TLE_Ctrl_Register_Read(HB_ACT_2_CTRL);
	if(temp != 0x06)
	{
		SP100_TLE_Ctrl_Register_Write(HB_ACT_2_CTRL, 0x06);		//--> OUTL5:ON & OUTL6:HZ
	}
}

/* SP100_Intake_Recycle -----------------------------------------------------*/
void SP100_Intake_Recycle(void)
{
	uint8_t temp;
	temp = SP100_TLE_Ctrl_Register_Read(HB_ACT_2_CTRL);
	if(temp != 0x09)
	{
		SP100_TLE_Ctrl_Register_Write(HB_ACT_2_CTRL, 0x09);		//--> OUTL5:HZ & OUTL6:ON
	}
}

/* SP100_Intake_Handler -----------------------------------------------------*/
void SP100_Intake_Handler(Intake_State state)
{
	switch (state)
	{
		case Intake_Fresh:
		{
			SP100_Intake_Fresh();
			break;
		}
		case Intake_Recycle:
		{
			SP100_Intake_Recycle();
			break;
		}
	}
}
