...
//	else
//		SP100_CCU_State = CCU_IGN_OFF;
	if(HAL_GPIO_ReadPin(AC_SW_GPIO_Port, AC_SW_Pin) == GPIO_PIN_RESET && HAL_GPIO_ReadPin(OFF_SW_GPIO_Port, OFF_SW_Pin) == GPIO_PIN_RESET){
		SP100_CCU_State = CCU_DIAG;
		SP100_LCD_ALL_SEG(SEG_ON);
		SP100_Delay(10000000);
		SP100_LCD_ALL_SEG(SEG_OFF);
	}
}
void SP100_Diag_Sensors(){
	if(SP100_DIAG_State != DIAG_Sensor) return;
}
void SP100_Diag_Actuator(){
	if(SP100_DIAG_State != DIAG_Actuator) return;
	SP100_LCD_Print_Temp(520);
	for(int i=0; i<10000; i++){
		SP100_Airmix_Handler(Airmix_05);
		SP100_Mode_Handler(Mode_Def);
	}
	int airmix_start_val = SP100_Airmix_FB;
	int mode_start_val = SP100_Mode_FB;
	for(int i=0; i<10000; i++){
		SP100_Airmix_Handler(Airmix_95);
		SP100_Mode_Handler(Mode_Face);
	}
	int airmix_end_val = SP100_Airmix_FB;
	int mode_end_val = SP100_Mode_FB;
	if(airmix_end_val - airmix_start_val > 1) SP100_Diag_Airmix_Lock_State = DIAG_ERROR;
	else SP100_Diag_Airmix_Lock_State = DIAG_OK;
	if(mode_end_val - mode_start_val > 1) SP100_Diag_Mode_Lock_State = DIAG_ERROR;
	else SP100_Diag_Mode_Lock_State = DIAG_OK;
...
void SP100_Save_Elements_State(){
	SP100_CCU_Elements_State.CCU_AC_State 		= SP100_AC_State;
	SP100_CCU_Elements_State.CCU_Blower_State = SP100_Blower_State;
	SP100_CCU_Elements_State.CCU_Intake_State = SP100_Intake_State;
	SP100_CCU_Elements_State.CCU_Mode_State   = SP100_Mode_State;
}
void SP100_Load_Elements_State(){
	SP100_AC_State 		 = SP100_CCU_Elements_State.CCU_AC_State;
	SP100_Blower_State = SP100_CCU_Elements_State.CCU_Blower_State;
	SP100_Intake_State = SP100_CCU_Elements_State.CCU_Intake_State;
	SP100_Mode_State   = SP100_CCU_Elements_State.CCU_Mode_State;
}
void SP100_CCU_Switch_Handler(uint16_t SW_Pin){
	...
	if(SW_Pin == Intake_SW_Pin)
	{
		...
		switch (SP100_CCU_State)
		{
			...
			case CCU_DIAG:
			{
				if(SP100_DIAG_State == DIAG_Sensor){
					if(SP100_DIAG_Sensor_Index == DIAG_Sensor_All)
						SP100_DIAG_Sensor_Index = DIAG_Sensor_Evap;
					else if(SP100_DIAG_Sensor_Index == DIAG_Sensor_Evap)
						SP100_DIAG_Sensor_Index = DIAG_Sensor_Cabin;
					else if(SP100_DIAG_Sensor_Index == DIAG_Sensor_Cabin)
						SP100_DIAG_Sensor_Index = DIAG_Sensor_Water;
					else if(SP100_DIAG_Sensor_Index == DIAG_Sensor_Water)
						SP100_DIAG_Sensor_Index = DIAG_Sensor_Ambient;
					else if(SP100_DIAG_Sensor_Index == DIAG_Sensor_Ambient)
						SP100_DIAG_Sensor_Index = DIAG_Sensor_All;
				}
				else{
					if(SP100_DIAG_Actuator_Index == DIAG_Actuator_All)
						SP100_DIAG_Actuator_Index = DIAG_Actuator_Airmix_Lock;
					else if(SP100_DIAG_Actuator_Index == DIAG_Actuator_Airmix_Lock)
						SP100_DIAG_Actuator_Index = DIAG_Actuator_Mode_Lock;
					else if(SP100_DIAG_Actuator_Index == DIAG_Actuator_Mode_Lock)
						SP100_DIAG_Actuator_Index = DIAG_Actuator_Airmix_Pot;
					else if(SP100_DIAG_Actuator_Index == DIAG_Actuator_Airmix_Pot)
						SP100_DIAG_Actuator_Index = DIAG_Actuator_Mode_Pot;
					else if(SP100_DIAG_Actuator_Index == DIAG_Actuator_Mode_Pot)
						SP100_DIAG_Actuator_Index = DIAG_Actuator_All;
				}
				break;
			}
			case CCU_IGN_OFF:
	...
	if(SW_Pin == Blower_Up_SW_Pin)
	{
		...
		switch (SP100_CCU_State)
		{
			...
			{
				if(SP100_DIAG_State == DIAG_Sensor) {
					SP100_DIAG_State = DIAG_Actuator;
					SP100_Diag_Actuator();
				}
				else {
					SP100_DIAG_State = DIAG_Sensor;
					SP100_Diag_Sensors();
				}
				break;
			}
	...
	if(SW_Pin == Airmix_Down_SW_Pin)
	{
		...
		case CCU_AUTO:
		{
			if(SP100_Set_Point > 21 && SP100_Set_Point <= 26) SP100_Set_Point -= 0.5;
	...
	if(SW_Pin == R_DEF_SW_Pin)
	{
		...
		*/
		...
		switch (SP100_CCU_State)
		{
			case CCU_OFF:
			{
				if(SP100_R_DEF_State == R_DEF_OFF)		{
					SP100_R_DEF_State = R_DEF_ON;
					SP100_RDEF_Start_Time = HAL_GetTick();
				}
				else if(SP100_R_DEF_State == R_DEF_ON)	{SP100_R_DEF_State = R_DEF_OFF;}
				break;
			}
...
void SP100_CCU_Preliminary(void){
	...
	// Intake Priliminary State 
	if(TS - TA >= 0) {
		if(TS - TC <= -1)    						 SP100_Intake_State = Intake_Fresh;
		else if(TS - TC >= 22.5)    		 SP100_Intake_State = Intake_Fresh;
		else if(TS-TC<22.5 && TS-TC>=-1) SP100_Intake_State = Intake_Recycle;
	}
	else {
		if(TS - TC < 1) 			SP100_Intake_State = Intake_Recycle;
		else if(TS - TC >= 1) SP100_Intake_State = Intake_Fresh;
	}
	// Blower Priliminary State
	if(Blower_Auto_Ctrl_State == Auto_Ctrl_ON){
		if(BLOWER_MODE == 0){
			...
			float v = 0;
			if(TS == 99 || TS == -99) v = SP100_Blower_Volt_L8;
			else if(cooling_state == 'A'){
				if(TW >= 65){
					if(TS-TC >= 1) 				v = absf(TS-TC)*0.4 + 6;
					else if(TS-TC <= -1)  v = absf(TS-TC)*0.4 + 6;
					else 								  v = 4;
				}
				else if(TW > 35 && TW < 65){
					if(absf(TS-TC) > 1) {
						if(TW/3.75 - 5.3 < absf(TS-TC)*0.4 + 6) 
								v = TW/3.75 - 5.3;//floor(0.2*TW - 5)*1.5 + 1;
						else
								v = absf(TS-TC)*0.4 + 6;
					}
					else											 v = 5;
				...
				if(TS-TA >= -5  && TS-TA < 2.5)  v = absf(TS-TC)*0.4 + 5;
				else if(TS-TA >= -10 && TS-TA < -5)   v = absf(TS-TC)*0.4 + 6;
				else if(TS-TA >= -20 && TS-TA < -10)  v = absf(TS-TC)*0.4 + 7;
				else if(TS-TA < -20) 								 v = absf(TS-TC)*0.4 + 8;
			}
			last_v = v;
			if( SP100_Blower_Dest_Volt1 - v > 1)
				SP100_Blower_Dest_Volt1 -= 0.9;
			else if( SP100_Blower_Dest_Volt1 - v > 0.5)
				SP100_Blower_Dest_Volt1 -= 0.4;
			
			if( SP100_Blower_Dest_Volt1 - v < -1)
				SP100_Blower_Dest_Volt1 += 0.9;
			else if( SP100_Blower_Dest_Volt1 - v < -0.5)
				SP100_Blower_Dest_Volt1 += 0.4;
			SP100_Blower_State = volt2state(SP100_Blower_Dest_Volt1);
		}
		// ------------ AC
...
void SP100_CCU_Auto_Calculations(void){
	...
	if(TS < 14) TS = -99;
	if(TS > 32) TS =  99;
	if(TS - TA > 2.5)  cooling_state = 'A';  // Winter
	if(TS - TA < -2.5) cooling_state = 'B';  // Summer
	
	// ------------ Intake
	if(Intake_Auto_Ctrl_State == Auto_Ctrl_ON){
		// Summer
		if(cooling_state == 'A'){ 
			if(TS - TC <= -2)  						 SP100_Intake_State = Intake_Fresh;
			else if(TS - TC >= 25)  			 SP100_Intake_State = Intake_Fresh;
			else if(TS-TC<=20 && TS-TC>=0) SP100_Intake_State = Intake_Recycle;
		}
		// Winter
		else if(cooling_state == 'B'){
	...
	if(Blower_Auto_Ctrl_State == Auto_Ctrl_ON){
		if(BLOWER_MODE == 0){
			...
			if(cooling_state == 'A'){
				if(TW >= 65){
					if(TS-TC >= 1) 				s = ceil(abs((int) (TS-TC)) + 0.1);
					else if(TS-TC <= -1)  s = ceil(abs((int) ((TS-TC)*0.4)) + 1);
					else 								  s = 1;
				}
				else if(TW > 35 && TW < 65){
			...
			if(TS == 99 || TS == -99) v = SP100_Blower_Volt_L8;
				...
				if(TS-TA >= -5  && TS-TA < 2.5)  v = absf(TS-TC)*0.4 + 5;
				else if(TS-TA >= -10 && TS-TA < -6)   v = absf(TS-TC)*0.4 + 6;
				else if(TS-TA >= -20 && TS-TA < -11)  v = absf(TS-TC)*0.4 + 7;
				else if(TS-TA < -21) 								 v = absf(TS-TC)*0.4 + 8;
				else v = last_v;
//				if(TS-TA >= -5  && TS-TA < 2.5)  v = absf(TS-TC)*0.4 + 4;
//				if(TS-TA >= -10 && TS-TA < -5)   v = absf(TS-TC)*0.4 + 5;
//				if(TS-TA >= -20 && TS-TA < -10)  v = absf(TS-TC)*0.4 + 6;
//				if(TS-TA < -20) 								 v = absf(TS-TC)*0.4 + 7;
			}			
			last_v = v;
			if( SP100_Blower_Dest_Volt1 - v > 1)
				SP100_Blower_Dest_Volt1 -= 0.9;
...
void SP100_CCU_Infinite_Loop(void){
	while(1)
	{
		...
		if(SP100_R_DEF_State == R_DEF_ON && HAL_GetTick() - SP100_RDEF_Start_Time > 1000 * 60 * 14){
